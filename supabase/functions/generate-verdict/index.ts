  ...
  if (key === 'rejected_fine') {
    ...
    const hint = proposal.creator_role === 'detective'
      ...
      : isGhost
        ...
        : ''
    const penaltyNote = isGhost ? '' : ', автор наказан -3 балла'
    return `Штраф отклонён${penaltyNote}. Адресат: ${name}. Обвиняемого не называй. Высмей ложный донос. Суть: ${text}.${hint}`
  }
