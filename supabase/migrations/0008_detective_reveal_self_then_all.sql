...
--
-- Раньше Детектив выбирал scope ('self'/'all') ДО того, как видел имя
-- автора — и каждый scope тратил отдельный заряд. Меняем порядок: сначала
-- 'self' (тратит 1 заряд, показывает имя только этому Детективу), и только
-- после этого можно эскалировать тот же кейс до 'all' — это публикация уже
-- узнанного факта, отдельного заряда не стоит, но требует, чтобы 'self' по
-- этому делу уже случился.
